...
import { createSolutionCacheIdentity, createTraceCacheIdentity } from '../src/services/cache/cacheIdentities';
...
test('reuses a validated Ideal Logic trace from persistent cache', async () => {
  process.env.VITE_OPENAI_API_KEY = 'test-openai-key';
  let traceCalls = 0;
  const provider = {
    id: 'openai',
  ...
  try {
    const fresh = await DynamicStepGenerator.generate(
      problem, stackApproach, source, problem.examples[0], 'typescript'
    );
    await cache.settlePendingWrites();
    const cached = await DynamicStepGenerator.generate(
      problem, stackApproach, source, problem.examples[0], 'typescript'
...
test('deduplicates concurrent generated-solution requests', async () => {
  process.env.VITE_OPENAI_API_KEY = 'test-openai-key';
  let calls = 0;
  ...
  try {
    const first = DynamicStepGenerator.generateSolution(problem, stackApproach, 'typescript');
    const second = DynamicStepGenerator.generateSolution(problem, stackApproach, 'typescript');
...
test('deduplicates concurrent Sync My Code trace requests', async () => {
  process.env.VITE_OPENAI_API_KEY = 'test-openai-key';
  let calls = 0;
  let release: (() => void) | undefined;
  const gate = new Promise<void>((resolve) => { release = resolve; });
  ...
  try {
    const first = DynamicStepGenerator.generateFromUserCode(
      problem, source, problem.examples[0], 'typescript'
    );
    const second = DynamicStepGenerator.generateFromUserCode(
      problem, source, problem.examples[0], 'typescript'
    );
    await Promise.resolve();
    release?.();
...
test('stores a fallback Ideal Logic trace under the provider that produced it', async () => {
  process.env.VITE_OPENAI_API_KEY = 'test-openai-key';
  process.env.VITE_CLAUDE_API_KEY = 'test-claude-key';
  const primary = {
    id: 'openai',
  ...
  try {
    await DynamicStepGenerator.generate(
      problem, stackApproach, source, problem.examples[0], 'typescript'
    );
    await cache.settlePendingWrites();
    const stored = await cache.listEntries();
    const fallbackIdentity = await createTraceCacheIdentity(
      problem, stackApproach.id, 'typescript', source, problem.examples[0], 'ideal',
